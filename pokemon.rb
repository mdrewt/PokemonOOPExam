...
class Pokemon
  ...
  def learn_attack(argument)
    attacks.push(argument)
  end
  
  def perform_attack(attack, target)
    target.receive_attack(attack.power, attack.type)
  end
  
  def receive_attack(damage, type)
    @hp -= damage
  end
...
class Attack
  def initialize(nexthash)
    @name = nexthash[:name]
    @power = nexthash[:power]
    @type = nexthash[:type]
  end
